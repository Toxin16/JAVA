...
221107 오디오
221108 계산기
221128 7장 제네리과 컬렉션 - 자료구조
 컬렉션은 요소들의 리스트나 집합을 관리하는 자료구조. 크기를 자동조절하여 크기에 구애받지 않고 요소의 추가, 삭제, 검색 등을 쉽게 할수있어 배열보다 용이함.
  주요 컬렉션 클래스에는 ArrayList<E>, HashMap<K, V>, Stack<E>등이 있다.
  Vector나 ArrayLIst등은 요소 객체들이 리스트 형식으로 순차적 저장, 인덱스로 검색할 수 있어 배열을 대신할 때 매우 효과적.
  HashMap 컬렉션에는 키-값의 쌍으로 저장, 키를 이용하여 요소가 검색됨. 삽입, 삭제, 검색 속도가 매우 빠름, 요소의 순서가 필요없으며 키를 이용하여 삽입하고 검색하는 응용에 적합.
  컬렉션은 JDK1.5이후 제네릭 기법으로 만들어짐.
  제네릭은 일반화시킨 타입을 사용하여 특정타입에 종속되지 않도록 클래스나 인터페이스, 메소드를 일반화시키는 기법. 제네릭에 구체적인 타입을 지정하여 특정타입으로 변신하여 사용가능.
  컬렉션은 구체적인 타입을 지정하여 사용. ex) Vector<Integer> v = new Vector<Integer>();
  컬렉션의 요소로는 객체들만 사용.
  int, char, double등의 기본 타입을 컬렉션의 요소로 삽입하려면 Wrapper 클래스를 이용하여 기본 타입을 객체로 만들어 사용.
  JDK1.5부터 자동 박싱 기능에 의해 기본타입의 값이 컬렉션에 삽입될 때 자동으로 Wrapper클래스로 포장되어 삽입하고, 추출될 때 자동 언박싱 기능에 의해 기본타입의 값으로 자동변환.
  Collection<E>인터페이스를 구현한 컬렉션의 경우 iterator()메소드를 호출하면 Iterator객체를 리턴하며, 이 객체를 이용하여 인덱스 없이 컬렉션의 요소에 대해 순차검색이 가능.
  Collections클래스는 컬렉션에 대해 요소 정렬, 검색, 최대 최솟값 구하기 등 다양한 유틸리티 메소드 제공.
  제네릭을 이용하면 컴파일 시에 타입이 결정되어 불필요한 오류를 방지하므로 안전한 프로그래밍이 가능.
  제네릭클래스 또는 인터페이스 선언은 타입 매개변수를 클래스나 인터페이스 이름 다음에 추가하여 선언한다.
  제네릭 타입으로 제네릭 클래스 내에서 객체와 배열을 생성할 수 없다.

230102 네트워크
 TCP/IP 프로토콜
  두 시스템 간 신뢰성 있는 데이터 전송을 관장하는 프로토콜. ex)e-mail, FTP, web(HTTP)
 IP
  인터넷 프로토콜
 포트
  통신하는 프로글매 간 가상의 연결단 포트 생성. 모든 응용프로그램은 하나 이상의 포트생성가능.+
